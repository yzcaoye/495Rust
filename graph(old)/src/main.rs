...
fn main() {
    ...
    graph.add_edge(String::from("a"),String::from("g"));
    graph.add_edge(String::from("b"),String::from("a"));
    graph.add_edge(String::from("b"),String::from("c"));
    graph.add_edge(String::from("c"),String::from("b"));
    ...
    graph.add_edge(String::from("e"),String::from("d"));
    graph.add_edge(String::from("e"),String::from("f"));
    graph.add_edge(String::from("f"),String::from("e"));
    graph.add_edge(String::from("g"),String::from("f"));
    graph.add_edge(String::from("g"),String::from("a"));
...
fn dfs(graph: &Adjlist, start: String, end: String, path: &mut Vec<String>){
    println!("start:{}  end:{}", start.clone(), end.clone());
    println!("{:?}", path.clone());
    ...
    for i in 0..vec.len(){
        ...
        if path.contains(&vec[i].clone())==false{
            ...
            // // println!("end:{}  vec[i]:{}", end.clone(), vec[i].clone());
            // if vec[i]==end.to_owned(){
            //     println!("arrive at the end! end:{}  vec[i]:{}", end.clone(), vec[i].clone());
            //     return;
            // }
            // else {
